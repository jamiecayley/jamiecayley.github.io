...
Theorem: let H be cyclic generated by x, then (1) if the order of x is infinite then $$H \cong \mathbb{Z}$$ (2) if $$|x| = n \in \mathbb{Z}_{> 0}$$ then $$H \cong \mathbb{Z}/n\mathbb{Z}$$.
...
Remark: if H and G are cyclic, $$H \cong G$$ if and only if $$|H| = |G|$$
...
Proof: (1) $$<x^a = \{x^n : n \in \mathbb{Z}\}$$. If $$x^b \in \{x^n : n \in \mathbb{Z}\}, x^b = x^{am} = b = am$$ so $$a|b$$. Similarly $$b | a$$ so $$|b| = |a|$$. Conversely if $$|a| = |b|$$ then $$b = \pm a$$ so $$x^b = x^a$$ or $$x^{-a}$$ and $$<^a> = <x^{-a}$$.
(2) $$<x^a> = <x^b>$$ so $$|x^a| = |x^b|$$ so $$\frac{n}/{(n,a)} = \frac{n}{(n,b)}$$ so (n,a) = (n,b). Conversely assume (n,a) = (n,b) = d we have a = kd, so $$x^a \in <x^d>$$ which means $$<x^d$$ and $$<x^a> \leq <x^d>$$. Since $$|<x^a| = \frac{n}{d}, |<x^d| = \frac{n}{(n,d)} = \frac{n}{d}$$ so $$<x^a> = <x^d>$$. Similarly $$<x^b = <x^d> = <x^a>$$.
